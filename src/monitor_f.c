...
int main(int argc, char* argv[]) {
   ...
   /* Real-time configuration */
    ...
    req.tv_nsec = (long)((period - req.tv_sec) * 1e9);
    ...
    while(!stop) {
        ...
        double sum = 0;
        for (int i = 0; i < np; i++) sum += local_samples[i];
        double avg = sum / np;

        /* Processing */
        for (int i = 0; i < np - 1; i++) {
            double curr_val = local_samples[i];
            double prev_val = local_samples[i + 1];

            /* Count zero crossings */
            if ((prev_val <= avg && curr_val > avg) || (prev_val >= avg && curr_val < avg)) {
                crossed++;
            }
        }
